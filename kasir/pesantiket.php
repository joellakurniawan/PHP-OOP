...
if(isset($_POST['purchase'])){
	...
	if ($jenispembayaran == 2){
		$sql = "SELECT * FROM customer WHERE nama_customer='".$_POST['name']."'";
		$result = mysqli_query($con,$sql);
		$hasil = mysqli_fetch_array($result);
		if ($inputno == $hasil['notlp_customer']){
			$idnya  = $hasil['id_customer'];
			$sql = "INSERT INTO pesanan VALUES (NULL, 1,".$jenispembayaran.", $idnya,".$_SESSION['idpegawai'].", ".$totalharga.")";
			mysqli_query($con,$sql);
			...
			$sql = "SELECT COUNT( t.id_tiket ) 
					...
					WHERE p.id_customer =$idnya";
			$result = mysqli_query($con,$sql);
			$hasil=mysqli_fetch_array($result);
			$poin = (int)$hasil[0] * 5;
			$sql = "UPDATE customer SET poin_customer=".$poin." WHERE id_customer=$idnya";
			mysqli_query($con, $sql);
			echo 'SUCCESS!';			
		}
		else {
		...
		$result = mysqli_query($con,$sql);
		$hasil = mysqli_fetch_array($result);
		if ($inputno == $hasil['nokredit_customer']){
			$idnya  = $hasil['id_customer'];
			$sql = "INSERT INTO pesanan VALUES (NULL, 1,".$jenispembayaran.", $idnya,".$_SESSION['idpegawai'].", ".$totalharga.")";
			mysqli_query($con,$sql);
			$sql = "UPDATE tiket SET status_tiket=1 WHERE id_pesanan=".$idpesanan."";
	...
	$(document).ready(function(){
		...
		$(".table").on("change","input:radio[name=metodepembayaran]", function() {
			...
			if (a == 'cashless'){
				...
				$(".inputnocash").prop('disabled', true);
				jenispembayaran = 2;
			}
			else if (a == 'credit') {
				...
				$(".inputnocash").removeAttr('disabled');
				$(".inputnohp").prop('disabled', true);
				$(".inputnokredit").prop('disabled', true);
				jenispembayaran = 1;
			}
		...
		$("#purchase").click(function(){
			...
			if (jenispembayaran == 2){
				...
				success: function(result){
					...
					if (result == "SUCCESS!"){
						$(".modal-footer").html('<button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="window.location.href=\'home.php\'">OK</button>');
					}
					else {
								...
								<div class="row">
									<div class="col-md-12">
										<div class="content first-content">
											...
											<button type="submit" class="btn btn-secondary" id="back" onclick="window.location.href='home.php'"> Back </button>
										</div>
									</div>
								</div>
							</div>                  
											...
											<div class="col-lg-4">
												<div class="input-group">
													<span class="input-group-addon">
														<input type="radio" name="metodepembayaran" value="cash"> Cash
													</span>
<!-- 													<input type="text" class="form-control inputnocash" disabled="true"> -->
												</div>
											</div>
											<div class="col-lg-4">
												<div class="input-group">
